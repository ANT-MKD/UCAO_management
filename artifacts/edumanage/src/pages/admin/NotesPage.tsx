...
export default function NotesPage() {
  ...
  const evaluationsPourCours = evaluations.filter(
    (e) => e.classeId === classeId && e.ecId === ecId && e.semestreId === semestreId,
  );
  ...
  const prefillFromEvaluation = (ev: EvaluationRecord | undefined) => {
    ...
    const existing = notes.filter((n) => n.classeId === ev.classeId && n.ecId === ev.ecId && n.type === type);
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {classeStudents.map((etu, i) => {
                  ...
                  const noteExistante = notes.find((n) => n.etudiantId === etu.id && n.classeId === classeId && n.ecId === ecId && n.type === noteType);
