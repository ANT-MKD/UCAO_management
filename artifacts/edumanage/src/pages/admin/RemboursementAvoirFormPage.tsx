...
import { rembourserAvoir } from "@/data/remboursementAvoirStore";
...
export default function RemboursementAvoirFormPage() {
  ...
  const [moyenRemboursement, setMoyenRemboursement] = useState("");
  ...
  const montantInsuffisant = Number(montant) > soldeAvoirDisponible;
  ...
  const canSubmit =
    ...
    !montantInsuffisant &&
    motif.trim().length > 0 &&
    moyenRemboursement.trim().length > 0;

  const handleSubmit = () => {
    if (!selectedStudent) return;
    if (!canSubmit) {
      toast.error("Sélectionnez un étudiant, un montant valide, un motif et un mode de règlement");
      return;
    }
    const result = rembourserAvoir({
    ...
    // debiterAvoir() mute l'objet étudiant en place : selectedStudent.soldeAvoir reflète déjà le nouveau solde ici.
    toast.success(`Remboursement ${result.record.reference} enregistré — nouveau solde avoir : ${formatCFA(selectedStudent.soldeAvoir)}`);
    setLocation(`/admin/avoir/remboursements/${result.record.id}`);
  };

  return (
    <div>
      <PageHeader
        breadcrumb={[{ label: "Admin" }, { label: "Finances" }, { label: "Remboursement avoir" }]}
        title="Nouveau remboursement avoir"
        subtitle="Rembourse en argent réel une partie ou la totalité du solde d'avoir d'un étudiant"
        actions={
          <button onClick={() => setLocation("/admin/avoir/remboursements")} className="flex items-center gap-2 px-4 py-2 border border-border rounded-xl text-sm hover:bg-muted transition-colors">
      ...
      <div className="max-w-xl mx-auto bg-card border border-border rounded-2xl p-6 space-y-5" style={{ boxShadow: "var(--shadow-sm)" }}>
            ...
            <>
              ...
              {filteredStudents.map((stu) => (
                <div
                  ...
                  data-testid={`remb-avoir-option-${stu.id}`}
                >
          ...
          <div>
            ...
            {selectedStudent && (
              <p className={cn("text-[11px] mt-1", montantInsuffisant ? "text-red-600 font-medium" : "text-muted-foreground")}>
                ...
                {montantInsuffisant && " — insuffisant pour ce montant"}
              </p>
            )}
          </div>
          <div>
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={() => setLocation("/admin/avoir/remboursements")} className="flex-1 py-3 border border-border rounded-xl font-medium hover:bg-muted transition-colors">
            Annuler
          </button>
          <button
            ...
            data-testid="remb-avoir-submit"
          >
            <Send size={15} /> Rembourser
          </button>
        </div>
      </div>
    </div>
