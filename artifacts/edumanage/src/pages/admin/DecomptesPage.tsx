...
export default function DecomptesPage() {
  ...
  const filtered = useMemo(() => {
    return decomptes
      .filter((d) => {
        ...
        if (filters.professeur && !d.professeur.toLowerCase().includes(filters.professeur.toLowerCase())) return false;
        if (filters.statut) {
          const statut = d.statut === "annule" ? "Annulé" : "Emis";
          if (statut !== filters.statut) return false;
        }
        if (dateDebut && d.date < dateDebut) return false;
        if (dateFin && d.date > dateFin) return false;
        if (montantMin && d.montantDecompte < Number(montantMin)) return false;
        if (montantMax && d.montantDecompte > Number(montantMax)) return false;
        if (typeFilter && d.type !== typeFilter) return false;
        return true;
      })
      .sort((a, b) => b.date.localeCompare(a.date));
  }, [decomptes, filters, dateDebut, dateFin, montantMin, montantMax, typeFilter]);
  ...
  const advancedActiveCount = [dateDebut, dateFin, montantMin, montantMax, typeFilter].filter(Boolean).length;

  const exportExcel = () => {
    const rows = filtered.map((d) => ({
      "N° décompte": d.reference,
      "Émis le": formatShortDate(d.date),
      "Pour le professeur": d.professeur,
      "Mt décompté": d.montantDecompte,
      "Mt payé": d.montantPaye,
      Statut: d.statut === "annule" ? "Annulé" : "Emis",
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Décomptes");
    XLSX.writeFile(wb, `decomptes-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };

  return (
    <div>
      <PageHeader
        breadcrumb={[{ label: "Admin" }, { label: "Finances" }, { label: "Les décomptes" }]}
        title="Les décomptes"
        subtitle={`${decomptes.length} décompte(s) émis`}
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={() => setLocation("/admin/decomptes/taux-horaire/new")}
              ...
              data-testid="btn-new-decompte-taux"
            >
              <Plus size={14} /> Décompte taux horaire
            </button>
            <button
              onClick={() => setLocation("/admin/decomptes/forfait/new")}
              className="flex items-center gap-2 px-3.5 py-2 bg-primary text-white rounded-xl text-xs font-medium hover:bg-primary/90 transition-colors"
              data-testid="btn-new-decompte-forfait"
            >
        ...
        <div className="bg-card border border-border rounded-xl p-4 mb-4 grid sm:grid-cols-2 lg:grid-cols-5 gap-3" style={{ boxShadow: "var(--shadow-sm)" }}>
          ...
          {advancedActiveCount > 0 && (
            <div className="sm:col-span-2 lg:col-span-5">
              <button
                onClick={() => { setDateDebut(""); setDateFin(""); setMontantMin(""); setMontantMax(""); setTypeFilter(""); setPage(1); }}
        ...
        <table className="w-full min-w-[900px] text-sm">
          <thead>
            <tr className="bg-muted/40 border-b border-border text-xs font-semibold text-muted-foreground uppercase tracking-wide">
              <th className="text-left px-4 py-3">N° décompte</th>
              <th className="text-left px-4 py-3">Émis le</th>
              <th className="text-left px-4 py-3">Pour le professeur</th>
              <th className="text-right px-4 py-3">Mt décompté</th>
            ...
            {pageRows.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-16 text-center text-sm text-muted-foreground">
                  {decomptes.length === 0
                    ? "Aucun décompte émis pour l'instant."
                    : "Aucun décompte ne correspond aux critères sélectionnés."}
                </td>
              </tr>
            ) : (
              pageRows.map((d) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">{d.professeur}</td>
                  <td className="px-4 py-3 text-right font-semibold">{formatCFA(d.montantDecompte)}</td>
                  <td className="px-4 py-3 text-right">{formatCFA(d.montantPaye)}</td>
                  <td className="px-4 py-3 text-center">
                    <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium", d.statut === "annule" ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700")}>
                      {d.statut === "annule" ? "Annulé" : "Emis"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setLocation(`/admin/decomptes/${d.id}`);
                      }}
                      className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-primary transition-colors"
                      aria-label="Voir le détail"
                      data-testid={`decompte-view-${d.id}`}
                    >
                      <Eye size={14} />
                    </button>
                  </td>
