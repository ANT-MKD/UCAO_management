...
import { matchProf } from "./TeacherPortalPages";
...
export default function TeacherRessourcesPage() {
  ...
  const mesEcs = useMemo(() => ecs.filter((e) => matchProf(e.responsable, currentUser?.name)), [ecs, currentUser?.name]);
  ...
  const ecsDeLaClasseChoisie = useMemo(() => {
  ...
  const handleFile = (file: File | undefined) => {
    ...
    reader.onload = () => {
      const ec = ecsDeLaClasseChoisie.find((e) => e.id === ecId);
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        <p className="text-sm text-muted-foreground mt-1">Déposez vos supports de cours pour les classes où vous enseignez</p>
      </div>
      ...
      {mesClasses.length === 0 ? (
        ...
        <>
          <div className="rounded-2xl border border-border bg-card p-5 grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1.5">Classe <span className="text-red-500">*</span></label>
              <select
                value={classeId}
          ...
          {!classeId ? (
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <label className="flex items-center gap-1.5 px-3 py-2 bg-primary text-white rounded-xl text-xs font-semibold hover:bg-primary/90 transition-colors cursor-pointer" data-testid="teacher-ressource-ajouter">
                  <Upload size={13} /> Ajouter un fichier
                  <input ref={inputRef} type="file" className="hidden" onChange={(e) => handleFile(e.target.files?.[0])} data-testid="teacher-ressource-input" />
                </label>
              </div>
              ...
              {ressources.length === 0 ? (
                ...
                <div className="space-y-2">
                  {ressources.map((r) => (
                    <div key={r.id} className="flex items-center gap-3 p-3.5 bg-muted/30 rounded-xl border border-border" data-testid={`teacher-ressource-ligne-${r.id}`}>
                      ...
                      <a href={r.dataUrl} download={r.nom} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-primary flex-shrink-0" data-testid={`teacher-ressource-telecharger-${r.id}`}>
                        <Download size={14} />
                      </a>
                      <button onClick={() => handleDelete(r.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-muted-foreground hover:text-red-600 flex-shrink-0" data-testid={`teacher-ressource-supprimer-${r.id}`}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ))}
