...
export default function RemboursementAvoirPage() {
  ...
  const filtered = useMemo(() => {
    return remboursements
      .filter((r) => {
        ...
        if (filters.encaissePar && !r.ajouteePar.toLowerCase().includes(filters.encaissePar.toLowerCase())) return false;
        if (filters.statut) {
          const statut = r.annulee ? "Annulé" : "Validé";
        ...
        if (moyenFilter && r.moyenRemboursement !== moyenFilter) return false;
        return true;
      })
      .sort((a, b) => b.date.localeCompare(a.date));
  }, [remboursements, filters, dateDebut, dateFin, montantMin, montantMax, moyenFilter]);
  ...
  const exportExcel = () => {
    const rows = filtered.map((r) => ({
      Numéro: r.reference,
      "Émis le": formatShortDate(r.date),
      Payeur: r.payeur,
      Montant: r.montant,
      "Encaissé par": r.ajouteePar,
      "Mode de règlement": r.moyenRemboursement,
      Statut: r.annulee ? "Annulé" : "Validé",
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Remboursements avoir");
    XLSX.writeFile(wb, `remboursements-avoir-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };

  return (
    <div>
      <PageHeader
        breadcrumb={[{ label: "Admin" }, { label: "Finances" }, { label: "Les remboursements" }]}
        ...
        actions={
          <div className="flex gap-2">
            ...
            <button
              onClick={() => setLocation("/admin/avoir/remboursements/new")}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-xl text-sm font-medium hover:bg-primary/90 transition-colors"
              data-testid="btn-new-remboursement"
            >
              ...
              <tr>
                <td colSpan={7} className="py-16 text-center text-sm text-muted-foreground">
                  {remboursements.length === 0
                    ...
                    : "Aucun remboursement ne correspond aux critères sélectionnés."}
                </td>
              </tr>
            ) : (
              pageRows.map((r) => {
                ...
                return (
                  <tr
                    ...
                    onClick={() => setLocation(`/admin/avoir/remboursements/${r.id}`)}
                    data-testid={`remb-row-${r.id}`}
                  >
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{r.ajouteePar}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setLocation(`/admin/avoir/remboursements/${r.id}`);
                        }}
                        className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-primary transition-colors"
                        aria-label="Voir le détail"
                        data-testid={`remb-view-${r.id}`}
                      >
                        <Eye size={14} />
                      </button>
                    </td>
